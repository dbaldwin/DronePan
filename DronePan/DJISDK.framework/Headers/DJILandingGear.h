...
/**
 *  This class contains the state of the landing gear. It also provides methods
...
/**
 *  Turns on the self-adaptive landing gear. If self-adaptive landing gear is
 *  turned on, the landing gear automatically transitions between deployed and
 *  retracted depending on altitude. During take-off, the transition point is
 *  1.2m above ground. After take-off (during flight or when landing), the
...
/**
 *  Turns off the self-adaptive landing gear. If self-adaptive landing gear is
 *  turned off, the aircraft will not automatically lower and raise the landing
 *  gear when the aircraft is 0.5m above the ground.
 *
 *  @param completion   Completion block that receives the execution result.
 */
...
/**
 *  Enters the transport mode. In transport mode, the landing gear will be in
 *  the same geometric plane as the aircraft body so it can be easily
 *  transported.
 *  It is only supported by Inspire 1 and Inspire 2.
 *
 *  @param completion   Completion block that receives the execution result.
 *  @attention If the gimbal is not removed or the ground is not flat, the enter
 *             transport mode will fail.
 */
 ...
 *  Only supported by Inspire 1 and Inspire 2.
 *  For Matrice 600 and Matrice 600 Pro, the landing gear cannot be controlled
 *  through the SDK, only automatically by the aircraft or manually by the
 *  remote controller.
 *
 *  @param completion   Completion block that receives the execution result.
 */
